<template>
    <tr  :class="this.$parent.isEdit(task.id) ? '' : 'd-none'"> 
        ...
        <td><input type="number" class="form-control mb-1" @input="checkPriorityValue"
            min="1" max="3" step="1" v-model="this.$parent.taskData.priority"></td>
    ...
    <tr :class="this.$parent.isEdit(task.id) ? '' : 'd-none'">
        <td colspan="5"><textarea cols="30" rows="5"  class="form-control mb-1" v-model="this.$parent.taskData.description"></textarea></td>
    </tr>
</template>
